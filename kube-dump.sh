...
usage() {
  ...
  echo "    -o ./downloads"
  echo "  # Creates: command-0, file-monitor-0, file-monitor-1 containers"
  echo ""
  ...
  echo "    --cri containerd --install-deps --verbose --no-glyphs \\"
  echo "    --download-verification none"
  echo ""
  echo "Container Naming:"
  echo "  Multiple -e flags: command-0, command-1, command-2, ..."
  echo "  Multiple -E flags: command-0, command-1, command-2, ... (node mode)"
  echo "  Multiple -s flags: file-monitor-0, file-monitor-1, ..."
...
#      - Downloads all files found by the select command (3 attempts with size verification)
      ...
      if [[ -n "$file_path" ]]; then
        ...
        while [[ $attempt -le $max_attempts && "$download_success" == "false" ]]; do
          ...
          if [[ -f "$output_file" ]]; then
            ...
            if [[ $attempt -eq $max_attempts ]]; then
              format_message_stderr "   ❌ $(basename "$file_path") - download failed after $max_attempts attempts"
              pod_had_failure=true
            else
              format_message_stderr "   ⚠️  Retrying $(basename "$file_path") - file not created (attempt $((attempt + 1))/$max_attempts)"
            fi
          fi
          ((attempt++))
        done
